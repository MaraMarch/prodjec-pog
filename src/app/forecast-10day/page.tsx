// src/app/forecast-10day/page.tsx
'use client';
import Link from 'next/link';
import { useEffect, useState, useCallback } from 'react';
import styles from './Forecast10Day.module.css'; // Убедитесь, что CSS файл существует
...
export default function Forecast10DayPage() {
  ...
  const fetchAndProcess = useCallback(async (cityToFetch: string) => {
    ...
    if (rawData) {
      const processed = processForecastDataForDays(rawData, 10); // API даст только 5
      if (processed) {
      ...
      <main className={styles.main}>
        <div className={styles.forecastCard}>
          <h1 className={styles.title}>
            ...
            Прогноз на 10 дней для {forecast?.city || cityName}
          </h1>
          ...
          {forecast && !loading && !error && (
             <>
            {forecast.days.length < 10 && forecast.days.length > 0 &&(
                   ...
                   <div className={styles.dayHeader}>
                    <h3 className={styles.dayDate}>
                      {new Date(day.date + 'T00:00:00').toLocaleDateString('ru-RU', {
                        weekday: 'short', // short для экономии места
                        day: 'numeric',
                        month: 'short'
                      })}
                    </h3>
